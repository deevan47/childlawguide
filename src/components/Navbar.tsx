      ...
      <div className="container mx-auto px-6 h-[72px] flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center h-full space-x-1">
          {NAVIGATION_ITEMS.map((item, index) => (
            <div key={index} className="relative group h-full flex items-center px-1">
              {/* Top Level Item */}
              <button 
              ...
              >
                ...
                {item.subItems && <ChevronDown size={14} className="opacity-70 group-hover:opacity-100 group-hover:rotate-180 transition-transform duration-300" />}
              </button>
                  ...
                  <div className="bg-white text-slate-800 shadow-2xl rounded-xl border border-slate-100 overflow-visible relative p-1">
                    <div className="flex flex-col">
                      {item.subItems.map((subItem, subIndex) => (
                        <div key={subIndex} className="relative group/sub rounded-lg">
                          <button
                          ...
                          >
                            ...
                            {subItem.subItems && (
                                <ChevronRight size={16} className="text-slate-400 group-hover/sub:text-blue-600 group-hover/sub:translate-x-1 transition-transform" />
                            )}
                          </button>
           ...
           <div className="container mx-auto px-6 py-8 flex flex-col space-y-4">
              {NAVIGATION_ITEMS.map((item, idx) => (
                <div key={idx} className="border-b border-blue-800/50 pb-2">
                  ...
                  >
                    <span>{item.label}</span>
                    {item.subItems && <ChevronDown className={`transition-transform duration-300 ${activeMobileSubmenu === item.label ? 'rotate-180' : ''}`} />}
                  </div>
                      ...
                      {item.subItems.map((sub, sIdx) => (
                        <div key={sIdx}>
                          <div 
                            ...
                            onClick={() => !sub.subItems && handleNavClick(sub.href)}
                          >
                             <span className={sub.subItems ? 'text-white font-bold' : 'text-blue-100'}>{sub.label}</span>
                          ...
                          {sub.subItems && (
                              <div className="pl-4 space-y-2 border-l-2 border-blue-500/30 mt-1 mb-2 ml-1">
                                  {sub.subItems.map((nested, nIdx) => (
                                      <div key={nIdx} className="py-2 text-sm text-blue-300 active:text-white cursor-pointer hover:text-white hover:bg-blue-800/30 px-2 rounded" onClick={() => handleNavClick(nested.href)}>
                                          {nested.label}
                                      </div>
                                  ))}
                              </div>
